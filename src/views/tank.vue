...
<div class="main" >
 ...
 <uploadPhoto :action="fullurl1"  
  explan="只能上传jpg/png文件,这为白底显示图片"
  ref="child" class="upload"></uploadPhoto>
 <uploadPhoto :action="fullurl2" 
  explan="只能上传jpg/png文件,这为黑底显示图片"
  ref="child1" class="upload"></uploadPhoto>
<div class="sub-div">
    <div class="message-div">
    <div class="upload-button-div" >
        <el-button class="upload-button" style="margin-left: 10px;" size="small" type="primary" @click="submitAll" :disabled="disabled" >上传到服务器</el-button>
    </div>
    ...
    <el-button class="download-button" style="margin-left: 10px;" size="small" type="primary"  :disabled= "disableDownload" @click="download">下载</el-button>
    <!-- <div class="clear"></div> -->
</div>
<div class="faq-div">
    <hr>
    ...
    <h3>处理原理是什么</h3>
    <p>这种图片的原理很简单，就是alpha 混合，一般图片每个像素点有三个属性（R,G,B）来表示，而png 格式的图片除了三原色三个分量以外，还有一个Alpha
    分量，也就是透明的，带有A分量的png 图片显示出来的颜色不仅和本身颜色有关也和背景颜色有关，这种关系叫做alpah 混合 </p>
     <p>alpha混合听上去很复杂，实际非常简单，其作用就是要实现一种半透明效果。假设一种不透明东西的颜色是A，另一种透明的东西的颜色是B，那么透过B去看A，看上去的颜色C就是B和A的混合颜色，可以用这个式子来近似
        <br> 设B物体的透明度为alpha(取值为0-1，0为完全透明，1为完全不透明) <br>
    R(C)=alpha*R(B)+(1-alpha)*R(A) <br>
    G(C)=alpha*G(B)+(1-alpha)*G(A) <br>
    B(C)=alpha*B(B)+(1-alpha)*B(A) <br> </p>
    <p> 由于处理的是黑白图片 所以三个分量变成了一个分量 L(灰度分量) <br>
    再黑白两个背景下我们显示的图片可以用一下两个公式表示 设白色为（1,1,1）黑色为(0,0,0)</p>
    <p>白底 <br>
    R(1)=alpha*L(mix)+(1-alpha)*1 <br>
    黑底 <br>
    L(2)=alpha*L(mix)+(1-alpha)*0 <br>
<p>
化简后就得到下面两个方程 <br>
L(1)=alpha*(1-L(mix))+1 <br>
L(2)=alpha*L(mix) <br>
图片的处理也就变成了已知 L1，L2,来解alpha 和mix 的值
 </p>
 <p>alpha=L(1)-L(2)+1 <br>
    L(mix)=L(2)/alpha
 </p>
    ...
    <h3>最大输出分辨率是多少 </h3>
    <p>最大长宽都为1800px 像素不超过5m 的图片，如果长宽有一边超过1800px
        就会被压缩</p>
    <h3>我上传的图片如何处理？</h3>
